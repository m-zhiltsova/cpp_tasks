...
public:
    ...
    void printHelp();
    void parseArguments(int argc, char* argv[], std::string& input_file, std::string& output_file, int& iterations);
    void initializeUniverse(Universe& universe, const std::string& input_file);
    void processCommands(Universe& universe);
private:
    std::string name;
    std::string rule;
    int width;
